...
const MIN_PX = 12
const clampPx = (v: number): number => Math.max(0, Math.min(innerWidth.value, v))
const toDate = (px: number): Date => (xScale.value.invert?.(px) as Date) ?? new Date()
...
const selection = computed<ChartBrushSelection>(
  () =>
    props.modelValue ?? props.initialSelection ?? { start: domain.value[0], end: domain.value[1] }
)
...
const leftDimStyle = computed(() => ({
  ...
  maskImage: props.fadeOuterEdges ? 'linear-gradient(to left, #000 85%, transparent)' : undefined
}))
const rightDimStyle = computed(() => ({
  ...
  backdropFilter: `blur(${props.blurPx}px)`,
  WebkitBackdropFilter: `blur(${props.blurPx}px)`,
  maskImage: props.fadeOuterEdges ? 'linear-gradient(to right, #000 85%, transparent)' : undefined
}))
  ...
  <foreignObject width="0" height="0">
    <Teleport v-if="containerRef" :to="containerRef">
      <div class="brush-layer">
        ...
        <div class="brush-window" :style="windowStyle" @pointerdown="beginDrag('move', $event)" />
        <div class="brush-handle" :style="leftHandleStyle" @pointerdown="beginDrag('left', $event)">
