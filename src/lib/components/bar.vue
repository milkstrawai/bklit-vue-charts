...
registerSeries({
  ...
  dotY: (datum: ChartDatum) => {
    const base = stacked.value
      ? (barConfig?.stackBase(datum, props.dataKey) ?? 0)
      : 0;
...
const staggerDelay = computed(
  () =>
    props.staggerDelay ??
    (data.value.length > 1
      ? (ENTER_DURATION_MS * 0.4) / 1000 / data.value.length
      : 0)
);
  ...
  <g v-if="!hidden">
    <template v-for="(bar, i) in bars" :key="bar.key">
      <motion.rect
        ...
        :animate="{ height: bar.height, y: bar.y, opacity: barOpacity(i), filter: 'blur(0px)' }"
