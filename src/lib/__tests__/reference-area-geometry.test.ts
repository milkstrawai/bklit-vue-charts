...
function baseOptions(overrides: Partial<Parameters<typeof computeReferenceAreaRect>[0]> = {}) {
...
describe('computeReferenceAreaRect', () => {
  ...
  it('clamps to the plot when ifOverflow is hidden', () => {
    expect(
      computeReferenceAreaRect(baseOptions({ y1: 150, y2: 250, ifOverflow: 'hidden' }))
    ).toEqual({ x: 0, y: 0, width: 400, height: 50 })
  })

  it('returns null when discard and partly outside the plot', () => {
    expect(
      computeReferenceAreaRect(baseOptions({ y1: 150, y2: 250, ifOverflow: 'discard' }))
    ).toBeNull()
  })

  it('does not clamp when ifOverflow is visible', () => {
    expect(
      computeReferenceAreaRect(baseOptions({ y1: 150, y2: 250, ifOverflow: 'visible' }))
    ).toEqual({ x: 0, y: -50, width: 400, height: 100 })
  })

  it('returns null for zero plot size', () => {
    expect(
      computeReferenceAreaRect(baseOptions({ innerWidth: 0, innerHeight: 200, y1: 10, y2: 20 }))
    ).toBeNull()
  })
})
